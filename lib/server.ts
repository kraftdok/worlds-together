...
export async function boundary(fn:()=>Promise<Response>){try{return await fn();}catch(e){if(e instanceof Problem)return json({error:e.message},e.status);if(e instanceof Error&&['The suggestion was incomplete. Your work is safe.','The suggestion could not be grounded in the supplied sources.'].includes(e.message))return json({error:e.message+' No proposal was saved. Choose the pieces again and retry.'},502);console.error('Worlds request failed:',e instanceof Error?e.name:'unknown',e instanceof Error?e.stack?.split('\n').slice(1,4).join('\n'):'');return json({error:'This action could not finish. Your draft is still here. Please retry.'},503);}}
