...
export default function CrossWorldDiscovery({own,user,api,close,openRoom,onPieces,focus,onSelection,initialIntention,addContext}:{initialIntention?:string;addContext:()=>void;focus?:Piece|null;onSelection?:(ids:string[])=>void;own:Piece[];user:string;api:(path:string,body?:unknown,method?:string)=>Promise<any>;close:()=>void;openRoom:(r:Room)=>void;onPieces:(p:Piece[])=>void}){
 ...
 {!data?<p role="status">{error||'Opening the space between worlds…'}</p>:mode==='pieces'?<>
  ...
  <div className="connection-first"><small>ASK FOR ONE USEFUL THING · EDIT BEFORE INVITING</small><EditableText value={chosen.firstStep} onChange={firstStep=>setChosen({...chosen,firstStep})} label="What should we make together first?" placeholder="A small draft we can review together, not a meeting to discuss a meeting…" maxLength={1200}/></div><p className="orbit-disclosure">This exact task and the text above go into the invitation. They must accept before a room opens. The agent can then prepare a draft for review; neither person is committed to doing the project.</p>
